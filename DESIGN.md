...
- **Finale atom:** a fixed decorative layer travels from the playground through the study rows and finale words, then lands in the footer wordmark's O slot and matches the footer text color. Tiles on each side of a row move together with an 8px clearance around the atom, then return. Reverse scrolling restores the original O. Reduced motion hides the moving layer, keeps the original O, and restores all tile positions.
